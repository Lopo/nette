...
class RoutingPanel extends Nette\Diagnostics\Panel
{
	...
	private function analyse($router)
	{
		...
		$this->routers[] = array(
			...
			'defaults' => $router instanceof Nette\Application\Routers\Route || $router instanceof Nette\Application\Routers\SimpleRouter ? $router->getDefaults() : array(),
			'mask' => $router instanceof Nette\Application\Routers\Route ? $router->getMask() : NULL,
			'request' => $request,
		);
	}
