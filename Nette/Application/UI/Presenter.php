 ...
 */
abstract class Presenter extends Control implements Nette\Application\IPresenter
{
	...
	public function run(Nette\Application\Request $request)
	{
		try {
			...
			if ($this->isAjax()) try {
				...
				if ($this->response instanceof Nette\Application\Responses\TextResponse && $this->isControlInvalid()) { // snippets - TODO
			...
			if ($this->hasFlashSession()) {
				$this->getFlashSession()->setExpiration($this->response instanceof Nette\Application\Responses\RedirectResponse ? '+ 30 seconds': '+ 3 seconds');
			}
	...
	public function changeAction($action)
	{
		if (Nette\StringUtils::match($action, "#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#")) {
			...
			throw new Nette\Application\BadRequestException("Action name '$action' is not alphanumeric string.");
		}
	}
	...
	{
		if ($destination instanceof Nette\Application\Request) {
			$this->sendResponse(new Nette\Application\Responses\ForwardResponse($destination));
		...
		$this->sendResponse(new Nette\Application\Responses\ForwardResponse($this->lastCreatedRequest));
	}
	...
	public function redirectUri($uri, $code = NULL)
	{
		...
		$this->sendResponse(new Nette\Application\Responses\RedirectResponse($uri, $code));
	}
	...
	public function canonicalize()
	{
		if (!$this->isAjax() && ($this->request->isMethod('get') || $this->request->isMethod('head'))) {
			...
			if ($uri !== NULL && !$this->getHttpRequest()->getUri()->isEqual($uri)) {
				$this->sendResponse(new Nette\Application\Responses\RedirectResponse($uri, Nette\Http\IResponse::S301_MOVED_PERMANENTLY));
			}
		}
	}
	...
	final protected function createRequest($component, $destination, array $args, $mode)
	{
		...
		if ($presenterFactory === NULL) {
			...
			$refUri = new Nette\Http\Url($this->getHttpRequest()->getUri());
		...
		$this->lastCreatedRequest = new Nette\Application\Request(
			$presenter,
	...
	private static function argsToParams($class, $method, & $args, $supplemental = array())
	{
		...
		if ($params === NULL) {
			$params = Nette\Reflection\Method::from($class, $method)->getDefaultParameters();
		}
		$i = 0;
		...
		if (array_key_exists($i, $args)) {
			$method = Nette\Reflection\Method::from($class, $method)->getName();
	...
	{
		return (array) Nette\Reflection\ClassType::from(/*5.2*func_get_arg(0)*//**/get_called_class()/**/)
