...
*/
abstract class BaseControl extends Nette\ComponentModel\Component implements Nette\Forms\IControl
{
	/** @var string */
	...
	{
		if (!$this->disabled && $form instanceof Nette\Forms\Form && $form->isAnchored() && $form->isSubmitted()) {
			$this->htmlName = NULL;
			$this->loadHttpData();
		}
	}
	...
	{
		return $this->addRule(Nette\Forms\Form::FILLED, $message);
	}
	...
	final public function isRequired()
	{
		foreach ($this->rules as $rule) {
			if ($rule->type === Nette\Forms\Rule::VALIDATOR && !$rule->isNegative && $rule->operation === Nette\Forms\Form::FILLED) {
				return TRUE;
			}
		}
		return FALSE;
		...
		foreach ($rules as $rule) {
			...
			if (is_array($rule->arg)) {
				foreach ($rule->arg as $key => $value) {
					$item['arg'][$key] = $value instanceof Nette\Forms\IControl ? (object) array('control' => $value->getHtmlName()) : $value;
				}
			} elseif ($rule->arg !== NULL) {
				$item['arg'] = $rule->arg instanceof Nette\Forms\IControl ? (object) array('control' => $rule->arg->getHtmlName()) : $rule->arg;
			}
	 ...
	 * @return bool
	 */
	public static function validateEqual(Nette\Forms\IControl $control, $arg)
	{
		$value = $control->getValue();
		foreach ((is_array($value) ? $value : array($value)) as $val) {
			foreach ((is_array($arg) ? $arg : array($arg)) as $item) {
				if ((string) $val === (string) ($item instanceof Nette\Forms\IControl ? $item->value : $item)) {
					return TRUE;
				}
			}
		}
